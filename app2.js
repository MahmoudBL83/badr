    ...
    var b = 0,htmlCode,
        ...
        d = {
            defaults: {
                ...
                event: { init: function () {}, update: function () {}, addArea: function () {}, removeArea: function () {} },
                input: {
                    active: {
            ...
            init: function (e, f) {
                ...
                if (!e||true) {
                    ...
                    (f = a.extend(!0, d.defaults, f)),
                        (c[b] = { state: { isLoaded: !1, areaIndex: 0, areaLength: 0 }, area: [], options: f }),
                        (e = b),
            ...
            update: function (b, e) {
                var f = this,
                    ...
                    e = e || {};
                if (b >= 0) {
                    (e = a.extend(!0, c[b].options, e)), (c[b] = { state: { isLoaded: !1, areaIndex: 0, areaLength: 0 }, area: [], options: e });
            ...
            bindEvents: function (b) {
                ...
                f.on("mousemove touchmove", function (f) {
                    ...
                    a.each(c[b].area[m].coords, function (a, b) {
                        var c = { naturalX: b.naturalX + Math.round((k.clientX - h.clientX) * l.ratioX), naturalY: b.naturalY + Math.round((k.clientY - h.clientY) * l.ratioY) };
            ...
            bindInputs: function (b) {
                ...
                a.each(c[b].options.input, function (f, g) {
                    var h = a(g.selector);
                    h.is("button")
                        ? a(document).on("click", g.selector, function () {
            ...
            getPosition: function (b, c, e) {
                ...
                return (
                    e
                        ? ((i.naturalX = Math.round((c.originalEvent.targetTouches[0].pageX - g.left) * h.ratioX)), (i.naturalY = Math.round((c.originalEvent.targetTouches[0].pageY - g.top) * h.ratioY)))
                        : ((i.naturalX = Math.round((c.clientX + (window.scrollX || window.pageXOffset) - g.left) * h.ratioX)), (i.naturalY = Math.round((c.clientY + (window.scrollY || window.pageYOffset) - g.top) * h.ratioY))),
                    i
                );
            },
            ...
            addArea: function (b, d) {
                1 == arguments.length && ((d = b), (b = a(this).data("imageMapper"))),
                    (c[b].area[c[b].state.areaLength] = { el: !1, shape: d, href: "", title: "", target: "", coords: []}),
            ...
            addCoord: function (a, b) {
                var d = c[a].state.areaIndex,
                    e = c[a].area[d].shape;
                    if(Math.round(b.naturalX / parseFloat(document.querySelector("#image-map").querySelector("img").style.transform.slice(6)))){
                        b.naturalX = Math.round(b.naturalX / parseFloat(document.querySelector("#image-map").querySelector("img").style.transform.slice(6))),
                        b.naturalY = Math.round(b.naturalY / parseFloat(document.querySelector("#image-map").querySelector("img").style.transform.slice(6)));
                    };
                ...
                f.css("width", "100%"),
                    ...
                    a.each(c[b].area, function (g, h) {
                        ...
                        a.each(h.coords, function (a, c) {
                            ...
                            if(false&&Math.round(f.clientX / parseFloat(document.querySelector("#image-map").querySelector("img").style.transform.slice(6)))){
                                scaledX = Math.round(f.clientX / parseFloat(document.querySelector("#image-map").querySelector("img").style.transform.slice(6))),
                                scaledY = Math.round(f.clientY / parseFloat(document.querySelector("#image-map").querySelector("img").style.transform.slice(6)));
                                j.push(scaledX, scaledY);
                            }
                            else
                            ...
                            h.el && (i = h.el),
                            "poly" == h.shape
                                ? (i || (i = a(document.createElementNS("http://www.w3.org/2000/svg", "polygon"))), i.attr("points", j.join(",")))
                                : "circle" == h.shape
                                ? j.length >= 4 &&
                                  (i || (i = a(document.createElementNS("http://www.w3.org/2000/svg", "circle"))), i.attr("cx", j[0]).attr("cy", j[1]), i.attr("r", Math.sqrt(Math.pow(j[2] - j[0], 2) + Math.pow(j[3] - j[1], 2))))
                                : j.length >= 4 &&
                                  (i || (i = a(document.createElementNS("http://www.w3.org/2000/svg", "rect"))),
                ...
                a.each(c[b].area, function (d, e) {
                    ...
                    if (
                        (a.each(e.coords, function (a, b) {
                            if(false&&Math.round(b.naturalX / parseFloat(document.querySelector("#image-map").querySelector("img").style.transform.slice(6)))){
                                scaledX = Math.round(b.naturalX / parseFloat(document.querySelector("#image-map").querySelector("img").style.transform.slice(6))),
                                scaledY = Math.round(b.naturalY / parseFloat(document.querySelector("#image-map").querySelector("img").style.transform.slice(6)));
                                f.push(scaledX, scaledY);
                            }
                            else
                    ...
                    i.attr("target", c[b].area[d].target),
                        ...
                        i.attr("idNum" ,document.querySelector("#pieceNum").value)
                        g.push(i[0].outerHTML);
                    localStorage.setItem("coords",f.join(","))
                }),
                    f.append("\n    " + g.join("\n    ") + "\n");
        ...
        a(document).on("init", function () {
            ...
            a(document).on("keydown keyup", "#image-mapper-url", function () {
                ...
                a(document).on("click", "#image-mapper-continue", function () {
                    var b = a("#image-mapper-dialog");
                    a("#image-map").imageMapper("update", { src: a("#image-mapper-url", b).val(), file: "" }), a("#image-mapper-load").modal("hide");
                }),
                a(document).on("change", "#image-mapper-file", function () {
                    var c = this.files[0],
                        d = new FileReader();
                    (d.onloadend =async function (d) {
                        var e = b(d.target.result);
                        a("#image-map").imageMapper("update", { src: "data:" + c.type + ";base64," + e, file: c.name });
                        console.log(document.querySelector("#image-map").querySelector("img").src);
                        let data = {
                            'html':document.querySelector("#image-map").querySelector("img").src,
                            'id':mapDiv.getAttribute('idNum')
                        }
                        await fetch(`/saveMapImage/`,{
                            method: 'POST',
                            headers: {
